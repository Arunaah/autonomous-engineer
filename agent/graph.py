...
def node_create_pr(state: EngineState) -> EngineState:
    ...
    pr = push_files(state["generated_files"], state["spec"]["title"], state["iteration"])
    pr_number = create_pull_request(pr["branch"], state["spec"])
...
def should_fix_or_deploy(state: EngineState) -> str:
    ...
    if state["iteration"] >= MAX_ITERATIONS:
        complete_run(state["run_id"], state["confidence"], state["iteration"], "failed")
        return "end"
    return "fix"
